...
class Vegas : public CCC {
  public:
    ...
    void onACK(int32_t ack) {
        ...
        if (m_bFirstAck) {
            m_bFirstAck = false;
            m_iLastUpdateAck = ack;
        } else {
            int32_t passed = ack - m_iLastUpdateAck;

            if (passed >= cwnd_pkts && m_dCurrentRTT > 0.0 && m_dBaseRTT > 0.0) {
                m_iLastUpdateAck = ack;

                double expected = cwnd_pkts / m_dBaseRTT;
                double actual = cwnd_pkts / m_dCurrentRTT;
                double diff = (expected - actual) * m_dBaseRTT;
