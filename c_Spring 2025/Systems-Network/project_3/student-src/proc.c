...
void proc_init(pcb_t *proc) {
    ...
    pfn_t pTable = free_frame();
    proc->saved_ptbr = pTable;
    memset(mem + (pTable * PAGE_SIZE), 0, PAGE_SIZE);

    fte_t *table_ent = proc->saved_ptbr + frame_table;
    table_ent->mapped = 1;
    table_ent->protected = 1;
    table_ent->process = proc;
}
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pte_t *table_ent = get_page_table_entry(i, proc->saved_ptbr, mem);
        if (table_ent->valid) {
            pfn_t pfn = table_ent->pfn;
    ...
    pfn_t pframe = proc->saved_ptbr;
    frame_table[pframe].protected = 0;
    frame_table[pframe].mapped = 0;
}
