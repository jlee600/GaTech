...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped) {
        ...
        pte_t *table_ent = get_page_table_entry(vpn, proc->saved_ptbr, mem);
        if (table_ent->dirty) {
            swap_write(table_ent, (void*) (mem + (victim_pfn * PAGE_SIZE)));
            table_ent->dirty = 0;
            stats.writebacks++;
        }
        table_ent->valid = 0;
...
pfn_t select_victim_frame() {
    ...
    if (replacement == RANDOM) {
        ...
        pfn_t victim = 0;
        uint8_t min_ref = UINT8_MAX;
        for (pfn_t i = 1; i <= num_entries; i++) {
            if (!frame_table[i].protected && frame_table[i].mapped) {
                if (frame_table[i].ref_count < min_ref) {
                    min_ref = frame_table[i].ref_count;
                    victim = i;
                }
            }
        }
        ...
        pfn_t candidate = (last_evicted + 1) % num_entries;
        while (frame_table[candidate].protected) {
            candidate = (candidate + 1) % num_entries;
        }
        last_evicted = candidate;
        return candidate;
    }
...
{
    /** FIX ME */
    uint8_t num_ent = MEM_SIZE / PAGE_SIZE;
    for (uint8_t i = 0; i < num_ent; i++) {
        // if (frame_table[i].mapped) {
        if (frame_table[i].mapped && !frame_table[i].protected) {
            pcb_t *proc = frame_table[i].process;
            pte_t *table_ent = get_page_table_entry(frame_table[i].vpn, proc->saved_ptbr, mem);
            // pte_t *table_ent = get_page_table(proc->saved_ptbr, mem); 
            uint8_t decayed = frame_table[i].ref_count >> 1;
            uint8_t new_ref = table_ent->referenced << 7;
            frame_table[i].ref_count = decayed | new_ref;
            table_ent->referenced = 0;
        }
    }
}
